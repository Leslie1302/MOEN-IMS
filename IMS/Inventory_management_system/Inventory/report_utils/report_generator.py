...
class WeeklyReportGenerator:
    ...
    def generate_report(self, user=None, custom_notes='', recipients=None, cc_recipients=None, dry_run=False):
        ...
        executive_summary = self._generate_executive_summary(
            commit_stats, categorized_commits, migrations
        )
        ...
        html_content, plain_text_content = self._generate_email_content(report, commit_stats)
    ...
    def _generate_executive_summary(self, commit_stats, categorized_commits, migrations):
        ...
        if total_commits > 0:
            summary_parts.append(f"This week saw {total_commits} commit{'s' if total_commits != 1 else ''} to the codebase")
        
        if features_count > 0:
            summary_parts.append(f"{features_count} new feature{'s' if features_count != 1 else ''} implemented")
        
        if fixes_count > 0:
            summary_parts.append(f"{fixes_count} bug fix{'es' if fixes_count != 1 else ''} applied")
        
        if migrations_count > 0:
            summary_parts.append(f"{migrations_count} database migration{'s' if migrations_count != 1 else ''} created")
        
        if not summary_parts:
            return "No significant development activity recorded this week."
    ...
    def _generate_email_content(self, report, commit_stats):
        """
